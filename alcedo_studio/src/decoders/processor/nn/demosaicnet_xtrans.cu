...
void XTransDemosaicNet::LoadWeightsImpl(const cuda::nn::SafetensorsTensorMap& tensors,
  ...
  for (int i = 1; i < kDepth; ++i) {
    ...
    std::vector<float> krsc(weight.data.size());
    cuda::nn::TransformConv2d3x3WeightsCutlassKrsc(weight.data.data(), kWidth, krsc.data());
    trunk_w_nhwc_[i] = cuda::nn::DeviceBufferF32(krsc.size());
    trunk_w_nhwc_[i].Upload(krsc, stream);
...
void XTransDemosaicNet::ForwardHwcChannelsLast(
  ...
  if (!weights_loaded() || output_w_cio_.get() == nullptr) {
    throw std::runtime_error("XTransDemosaicNet::ForwardHwcChannelsLast: weights not loaded");
  }
  if (rgb_hwc == nullptr) {
    throw std::runtime_error("XTransDemosaicNet::ForwardHwcChannelsLast: null rgb pointer");
  }
  RequireContiguousNchw3(input, "XTransDemosaicNet::ForwardHwcChannelsLast input");
  ...
  if (N != 1 || (H % kPackFactor) != 0 || (W % kPackFactor) != 0 || H < kMinSpatial ||
      W < kMinSpatial) {
    throw std::runtime_error("XTransDemosaicNet::ForwardHwcChannelsLast: invalid input geometry");
  }
  ...
  if (slots.estimate_bytes == 0) {
    throw std::runtime_error(
        "XTransDemosaicNet::ForwardHwcChannelsLast: invalid activation geometry");
  }
  ...
  if (slot_a == nullptr || slot_b == nullptr) {
    throw std::runtime_error(
        "XTransDemosaicNet::ForwardHwcChannelsLast: failed to allocate activation slots");
  }
  ...
  for (int i = 1; i < kDepth; ++i) {
    ...
    cuda::nn::Conv2d3x3NhwcCutlassBiasRelu(cur_base, next_base, trunk_w_nhwc_[i].get(),
  ...
  if (cat_bytes > slots.trunk_slot_bytes) {
    throw std::runtime_error(
        "XTransDemosaicNet::ForwardHwcChannelsLast: concat exceeds trunk slot");
  }
