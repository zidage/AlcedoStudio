...
// Qt has no rotate cursor shape: a clockwise 270-degree arc with an arrowhead
// at each end, white over a dark outline like the platform resize cursors.
[[nodiscard]] auto MaskRotateCursor() -> QCursor {
  constexpr double kPi          = 3.14159265358979323846;
...
void EditorOverlayItem::applyMaskCursor() {
  const MaskOverlayCursor next = MaskOverlayCursorForDisplay(mask_display_);
  if (next == mask_cursor_) {
    return;
  }
  mask_cursor_ = next;
  switch (next) {
    case MaskOverlayCursor::None:
      unsetCursor();
      break;
    case MaskOverlayCursor::Move:
      setCursor(Qt::SizeAllCursor);
      break;
    case MaskOverlayCursor::Rotate: {
      static const QCursor rotate = MaskRotateCursor();
      setCursor(rotate);
      break;
    }
    case MaskOverlayCursor::ResizeHorizontal:
      setCursor(Qt::SizeHorCursor);
      break;
    case MaskOverlayCursor::ResizeVertical:
      setCursor(Qt::SizeVerCursor);
      break;
    case MaskOverlayCursor::ResizeDiagonalDown:
      setCursor(Qt::SizeFDiagCursor);
      break;
    case MaskOverlayCursor::ResizeDiagonalUp:
