...
#include "ui/alcedo_main/album_backend/editor_session_production_journal_writer_port.hpp"
...
EditorSessionProductionJournalWriterPort::EditorSessionProductionJournalWriterPort(
    Services services)
    : services_(std::move(services)) {}

EditorSessionProductionJournalWriterPort::~EditorSessionProductionJournalWriterPort() = default;

void EditorSessionProductionJournalWriterPort::SetServices(Services services) {
...
auto EditorSessionProductionJournalWriterPort::HasJournalPathResolver() const -> bool {
  std::scoped_lock lock(mutex_);
...
auto EditorSessionProductionJournalWriterPort::ImageLockFor(sl_element_id_t element_id)
...
auto EditorSessionProductionJournalWriterPort::WriterFor(sl_element_id_t element_id,
...
// ── Finalize / commit / discard ─────────────────────────────────────────────

auto EditorSessionProductionJournalWriterPort::FinalizeEdit(sl_element_id_t /*element_id*/,
                                                            std::uint64_t /*session_generation*/,
                                                            std::string* /*error*/) -> bool {
  return true;
}

auto EditorSessionProductionJournalWriterPort::CommitJournal(sl_element_id_t element_id,
                                                             std::uint64_t   session_generation,
                                                             std::string*    error)
...
auto EditorSessionProductionJournalWriterPort::CommitJournalAsync(
...
auto EditorSessionProductionJournalWriterPort::DiscardUnflushed(sl_element_id_t element_id,
                                                                std::string*    error) -> bool {
  const auto                                   image_lock = ImageLockFor(element_id);
  std::scoped_lock                             image_guard(*image_lock);
...
auto EditorSessionProductionJournalWriterPort::RecordEdit(
    sl_element_id_t element_id, std::uint64_t session_generation,
    const alcedo::EditTransaction& transaction, std::string* error) -> bool {
  const auto       image_lock = ImageLockFor(element_id);
  std::scoped_lock image_guard(*image_lock);
...
auto EditorSessionProductionJournalWriterPort::RecordCursorMove(sl_element_id_t element_id,
                                                                std::uint64_t   session_generation,
...
auto EditorSessionProductionJournalWriterPort::RecordRewriteTimeline(
    sl_element_id_t element_id, std::uint64_t session_generation,
