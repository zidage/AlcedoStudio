...
Item {
    ...
    ShaderEffectSource {
        ...
        sourceRect: root.blurSource ? Qt.rect(0, 0, root.blurSource.width, root.blurSource.height) : Qt.rect(0, 0, 0, 0)
        textureSize: root.blurSource ? Qt.size(Math.max(1, root.blurSource.width), Math.max(1, root.blurSource.height)) : Qt.size(1, 1)
        live: true
        recursive: false
        hideSource: false
        visible: false
