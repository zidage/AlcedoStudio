...
EditorSessionService::EditorSessionService(Dependencies dependencies)
    : dependencies_(std::move(dependencies)),
      lifecycle_(EditorSessionLifecycle::Dependencies{dependencies_.pipeline,
                                                       dependencies_.history}),
      save_service_(EditorSaveCheckpointService::Dependencies{dependencies_.journal,
                                                              dependencies_.tasks}),
      ...
      render_(EditorSessionRenderController::Dependencies{
          dependencies_.render, lifecycle_,
...
auto EditorSessionService::Patch(EditorAdjustmentPatch patch) -> EditorSessionResult {
  ...
  render_.RouteInitialRender(outcome.render_command);
  EditorSessionResult result;
  result.kind     = EditorSessionResultKind::RenderRouted;
  result.state    = lifecycle_.state();
  result.identity = lifecycle_.identity();
...
auto EditorSessionService::Discard() -> EditorSessionResult {
  ...
  render_.RouteInitialRender(outcome.render_command);
  EditorSessionResult result;
  result.kind     = (state == EditorSessionState::Failed)
                        ? EditorSessionResultKind::StateChanged
...
auto EditorSessionService::RequestViewChange(EditorRenderReason                  reason,
  ...
  const auto event   = render_.RouteViewChange(command);
  EditorSessionResult result;
  result.state    = event.state;
  result.identity = event.identity;
  switch (event.kind) {
...
auto EditorSessionService::RecordTimelineRewrite(const Hash128&  expected_timeline_hash,
                                                  ...
                                                  std::string*           error) -> bool {
  return edit_.RecordTimelineRewrite(expected_timeline_hash, discarded_tail_hash,
                                     retained_cursor, replacement, lifecycle_.identity(),
                                     error);
}

void EditorSessionService::NotifyImageAcquired(std::uint64_t session_generation, bool success,
                                               std::string message) {
  if (!lifecycle_.MatchesIdentity(0, 0, session_generation)) {
    return;
  }
  const auto state = lifecycle_.state();
...
void EditorSessionService::NotifySaveFinished(std::uint64_t session_generation, bool success,
  ...
  result.checkpoint_completed = success;
  result.error               = std::move(message);
  save_service_.OnCheckpointFinished(result);
  NotifyChange();
}
