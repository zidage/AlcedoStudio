...
static inline float3 ApplyHls(float3 c, __global const GradeAdjustmentParams* p) {
  ...
  for (int i = 0; i < 8; ++i) {
    const float difference = fabs(hue - p->values[i]);
    const float distance   = fmin(difference, 360.0f - difference);
    const float width      = fmax(p->values[32 + i], 1.0f);
  ...
  if (fabs(adj_h) <= 1.0e-6f && fabs(adj_l) <= 1.0e-6f && fabs(adj_s) <= 1.0e-6f) return c;
