...
[[nodiscard]] inline auto MakeBayerStudentTilePolicy() -> CudaTilePolicy {
  constexpr int owned = BayerDemosaicNet::kTileOutput;  // 1024
  const int input     = owned + 2 * BayerDemosaicNet::kTileBorder;
  const int step      = StudentPeriodSafeStep(owned, BayerDemosaicNet::kCfaPeriod);
  CudaTilePolicy p;
...
[[nodiscard]] inline auto MakeXTransStudentTilePolicy() -> CudaTilePolicy {
  constexpr int owned = XTransDemosaicNet::kTileOutput;  // 1024
  const int input     = owned + 2 * XTransDemosaicNet::kTileBorder;
  const int step      = StudentPeriodSafeStep(owned, XTransDemosaicNet::kCfaPeriod);
  CudaTilePolicy p;
  p.input_tile    = {input, input};
  p.output_tile   = {owned, owned};
  p.step          = {step, step};
