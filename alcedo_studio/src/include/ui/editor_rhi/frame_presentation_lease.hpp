...
enum class LeaseWritableResourceKind : std::uint8_t {
  ...
  MetalTexture,  // same as presentation handle for Metal (Phase 8)
};
...
struct WritableTargetRequest {
  ...
  [[nodiscard]] auto valid() const -> bool {
    return dimensions.valid() && image_generation != 0;
  }
};
...
struct LeaseLifetimeToken {
  ...
  void MarkProducerComplete() {
    producer_complete.store(true, std::memory_order_release);
  }
  void MarkRendererComplete() {
    renderer_complete.store(true, std::memory_order_release);
  }
