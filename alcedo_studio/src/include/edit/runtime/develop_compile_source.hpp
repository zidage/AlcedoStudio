...
struct DevelopCompileSource {
  ...
  /// Oriented student-tiled Neural Engine output. Empty when that demosaic cannot run.
...
inline auto operator==(const DevelopCompileSource& a, const DevelopCompileSource& b) -> bool {
  return a.kind == b.kind && a.host_extent == b.host_extent &&
         ...
         a.neural_output_extent == b.neural_output_extent &&
         a.neural_full_reference_extent == b.neural_full_reference_extent &&
         a.sensor_active_area == b.sensor_active_area && a.downsample_passes == b.downsample_passes;
}
