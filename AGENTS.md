...
| `gate`, `gates`, `gated`, `gating`, and compound identifiers such as `InFlightGate` | Project-authored identifiers, tests, comments, documentation, plans, commit and pull request text, and user-facing text | Name the exact item: acceptance criteria, completion criteria, requirement, precondition, standard, checklist, check, limit, lock, admission rule, or concurrency limit. Existing matches are renamed when their file is touched. |
