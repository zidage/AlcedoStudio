...
description: Use when building the LibRaw submodule (src/third_party/LibRaw) on Windows/MSVC — i.e. when the user mentions LibRaw, libraw.dll, libraw_static.lib, raw-identify, dcraw_emu, Makefile.msvc, or asks to compile/build the third-party LibRaw tree. LibRaw has NO in-repo CMakeLists.txt; it builds via nmake + Makefile.msvc.
---

# LibRaw MSVC Build

Build the LibRaw third-party library (`src/third_party/LibRaw`) on Windows with the MSVC toolchain. This is the build path that actually works in this repo — there is **no** `CMakeLists.txt` inside the LibRaw tree (per LibRaw's `README.cmake`, the CMake scripts live in a separate `LibRaw/LibRaw-cmake` repo), so do **not** attempt `cmake` here. Use `nmake -f Makefile.msvc`.
...
```
src/third_party/LibRaw/msvc_build.cmd
```
...
Run from the LibRaw directory (`src/third_party/LibRaw`). Use the `cmd` form so the `.cmd` wrapper executes correctly:

- Build everything (default `all` = DLL + static lib + all sample exes):
  ```
  cmd /c "src\third_party\LibRaw\msvc_build.cmd"
  ```
- Build only the static library:
  ```
  cmd /c "src\third_party\LibRaw\msvc_build.cmd" lib\libraw_static.lib
  ```
- Build only the DLL (+ import lib):
  ```
  cmd /c "src\third_party\LibRaw\msvc_build.cmd" bin\libraw.dll
  ```
- Clean:
  ```
  cmd /c "src\third_party\LibRaw\msvc_build.cmd" clean
  ```
...
After a full build (paths relative to `src/third_party/LibRaw`):
...
The sample exes link against `libraw.dll`. A quick smoke test: `bin\raw-identify.exe` (no args) should print a usage banner — if it runs, the DLL loads/links correctly.
...
If `src/third_party/LibRaw/msvc_build.cmd` is absent, recreate it. It must, from the LibRaw directory:
...
Reference implementation is committed at `src/third_party/LibRaw/msvc_build.cmd`.
